    ...
    UI_FloatRange(minValue = 1f, maxValue = 60f, stepIncrement = 0.5f, scene = UI_Scene.All)]
        ...
        */
            ...
            if (vessel.isActiveVessel && !guardMode) // Manual firing.
            {
                ...
                if (selectedWeapon != null &&
                    (selectedWeapon.GetWeaponClass() == WeaponClasses.Missile
                     ...
                     || selectedWeapon.GetWeaponClass() == WeaponClasses.Drone
                    ))
                {
                    canRipple = true;
        ...
        private void CalculateMissilesAway() //FIXME - add check for identically named vessels
        {
            ...
            /*
            int tempDronesAway = 0;
            using (List<IBDWeapon>.Enumerator firedDrones = BDATargetManager.FiredDrones.GetEnumerator())
                while (firedDrones.MoveNext())
                {
                    if (firedDrones.Current == null) continue;

                    var Drone = firedDrones.Current as ModuleDrone;

                    if (Drone.SourceVessel != this.vessel) continue;

                    if (Drone.MissileState != MissileBase.MissileStates.PostThrust && !Drone.HasExploded)
                    {
                        tempDronesAway++;
                    }
                }

            this.dronesAway = tempDronesAway;
            */
        }
        ...
        void OnGUI()
        {
                ...
                BDArmorySetup.GAME_UI_ENABLED && !MapView.MapIsEnabled)
            {
                ...
                if (missile) //FIXME - convert to a switch{case} implementation
                {
                            ...
                            if (mlauncher != null)
                            {
                                ...
                                if (ml && mlauncher.multiLauncher && mlauncher.multiLauncher.turret && antiRadTargetAcquired)
                                {
                                    ...
                                    while (antiRadTargetAcquired && Time.time - turretStartTime < Mathf.Max(targetScanInterval / 2f, 2))
                                    {
                                        float angle = Vector3.Angle(mlauncher.multiLauncher.turret.finalTransform.forward, mlauncher.multiLauncher.turret.slavedTargetPosition - mlauncher.multiLauncher.turret.finalTransform.position);
                                        mlauncher.multiLauncher.turret.slaved = true;
                                        mlauncher.multiLauncher.turret.slavedTargetPosition = MissileGuidance.GetAirToAirFireSolution(mlauncher, antiRadiationTarget, guardTarget.Velocity());
                                        mlauncher.multiLauncher.turret.SlavedAim();
        ...
        public void UpdateList()
        {
            ...
            using (var weapon = VesselModuleRegistry.GetModules<IBDWeapon>(vessel).GetEnumerator())
                while (weapon.MoveNext())
                {
                    ...
                    if ((weapon.Current.GetWeaponClass() == WeaponClasses.Gun || weapon.Current.GetWeaponClass() == WeaponClasses.Rocket || weapon.Current.GetWeaponClass() == WeaponClasses.DefenseLaser) &&
                        weapon.Current.GetPart().FindModuleImplementing<ModuleWeapon>().DroneWeapon)
                    {
                        continue;
                    }
                    if (!alreadyAdded)
                    ...
                    weapon.Current.GetWeaponClass() == WeaponClasses.Drone ||
        ...
        private void UpdateSelectedWeaponState()
        {
            ...
            if (selectedWeapon != null && (selectedWeapon.GetWeaponClass() == WeaponClasses.Bomb || selectedWeapon.GetWeaponClass() == WeaponClasses.Missile || selectedWeapon.GetWeaponClass() == WeaponClasses.SLW || selectedWeapon.GetWeaponClass() == WeaponClasses.Drone))
            {
        ...
        }//FIXME - deployables for MMG/drones
        ...
        private MissileBase GetAsymMissile()
        {
            ...
            if (weaponArray[weaponIndex].GetWeaponClass() == WeaponClasses.Bomb ||
                ...
                weaponArray[weaponIndex].GetWeaponClass() == WeaponClasses.Drone ||
                weaponArray[weaponIndex].GetWeaponClass() == WeaponClasses.SLW)
            {
                ...
                using (var ml = VesselModuleRegistry.GetModules<MissileBase>(vessel).GetEnumerator())
                    while (ml.MoveNext())
                    {
                        ...
                        {
                            ModuleDrone drone = ml.Current as ModuleDrone;
                            if (drone != null)
                            {
                                if (weaponArray[weaponIndex].GetPart() == null || drone.part.name != weaponArray[weaponIndex].GetPart().name) continue;
                            }
                            else
                            {
        ...
        private MissileBase GetRotaryReadyMissile()
        {
            ...
            if (weaponArray[weaponIndex].GetWeaponClass() == WeaponClasses.Bomb ||
                ...
                weaponArray[weaponIndex].GetWeaponClass() == WeaponClasses.SLW)
            {
                //TODO BDModularGuidance, ModuleDrone: Implement rotaryRail support
                    ...
                    while (item.MoveNext())
                    {
                        ...
                        if (candidateClass == WeaponClasses.Missile)
                        {
                            ...
                            { //is modular missile
                                BDModularGuidance mm = item.Current as BDModularGuidance; //need to work out priority stuff for MMGs
                                candidateTDPS = 5000;
                                candidateDetDist = mm != null ? mm.warheadYield : 10;
        ...
        void GuardMode()
        {
            ...
            if (Time.time - targetScanTimer > targetScanInterval)
            {
                ...
                if (!guardFiringMissile)// || (firedMissiles >= maxMissilesOnTarget && multiMissileTgtNum > 1 && BDATargetManager.TargetList(Team).Count > 1)) //grab new target, if possible
                {
                    ...
                    if (weaponIndex > 0)
                    {
                        if (selectedWeapon.GetWeaponClass() == WeaponClasses.Missile || selectedWeapon.GetWeaponClass() == WeaponClasses.SLW || selectedWeapon.GetWeaponClass() == WeaponClasses.Drone)
                        {
        ...
        public bool GetLaunchAuthorization(Vessel targetV, MissileFire mf)
        {
            ...
            if (missile != null && targetV != null)
            {
                ...
                if (launchAuthorized)
                {
                    ...
                    launchAuthorized = launchAuthorized && (Vector3.Angle(missile.GetForwardTransform(), futurePos - myFuturePos) < (unguidedWeapon ? 5 : missile.maxOffBoresight * boresightFactor)); // Launch is likely also possible at fTime
                }
            }
