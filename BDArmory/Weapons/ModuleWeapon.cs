    ...
    public class ModuleWeapon : EngageableWeapon, IBDWeapon
    {
                        ...
                        if ((!useRippleFire || fireState.Length == 1) || (useRippleFire && i == barrelIndex))
                        {
                            if (CanFire(requestResourceAmount))
                            {
                                ...
                                FireBullet(bulletInfo, ProjectileCount, sourceInfo, graphicsInfo, nukeInfo,
                                    ...
                                    true, 0f, 0f, fireTransform.forward, false, maxDeviation, targetV, guidance, true, cordiumDuration, hurtDiameter, cordiumDPS, beamScalar);
        ...
        void UpdateTargetVessel()
        {
            ...
            if (weaponManager)
            {
                ...
                if ((turret && (maxPitch != minPitch || yawRange > 0)) && weaponManager.mainTGP != null && ModuleTargetingCamera.windowIsOpen && weaponManager.mainTGP.slaveTurrets && weaponManager.slavedPosition != Vector3.zero)
                {
