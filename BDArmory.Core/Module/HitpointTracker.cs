...
namespace BDArmory.Core.Module
{
    public class HitpointTracker : PartModule
    {
        ...
        private const int HpRounding = 25; //was 100
        ...
        public float CalculateTotalHitpoints()
        {
            ...
            if (!part.IsMissile())
            {
				...
				{
					hitpoints = (part.mass * 1000f) * 3.5f; // since wings are basically a 2d object, lets have mass be our scalar - afterall, 2x the mass will ~= 2x the surface area
				}
				if (part.IsAero())
				{
