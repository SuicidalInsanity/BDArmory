...
namespace BDArmory.Modules
{
	public class ModuleWeapon : EngageableWeapon, IBDWeapon
	{
		...
		[KSPField(isPersistant = true, guiActive = true, guiName = "Weapon Group Name ", guiActiveEditor = true), UI_Label(affectSymCounterparts = UI_Scene.All, scene = UI_Scene.All)]
		...
		{
			if ((turret || eWeaponType != WeaponTypes.Rocket) || (eWeaponType == WeaponTypes.Rocket && externalAmmo))
			{
				return;
			}
			part.decouple(0);
		...
		public void RunTrajectorySimulation()
		{
				...
				(BDArmorySettings.DRAW_DEBUG_LINES || (vessel && vessel.isActiveVessel && !aiControlled && !MapView.MapIsEnabled && !pointingAtSelf && eWeaponType != WeaponTypes.Rocket)))
			{
				...
				if (eWeaponType == WeaponTypes.Rocket && !externalAmmo)
				{
	...
	public class WeaponGroupWindow : MonoBehaviour
	{
		...
		private string txtName2 = string.Empty;
