...
UI_Toggle(enabledText = "Reckless", disabledText = "Standard", scene = UI_Scene.All),]
		...
		void UpdateUI()
		{
			...
			Fields["ABSpeed"].guiActive = advancedSettings;
			Fields["ABSpeed"].guiActiveEditor = advancedSettings;
        ...
        void FlyToPosition(FlightCtrlState s, Vector3 targetPosition)
        {
            ...
            else//(steerMode == SteerModes.Aiming)
            {
				...
				targetDirectionYaw = vesselTransform.InverseTransformDirection(vessel.Velocity()).normalized;
				targetDirectionYaw = Vector3.RotateTowards(Vector3.up, targetDirectionYaw, 25 * Mathf.Deg2Rad, 0); // lets actually make the Ai use yaw control auth
			}
