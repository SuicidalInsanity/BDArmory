...
namespace BDArmory.Modules
{
    public class MissileFire : PartModule
    {
        ...
        [KSPEvent(guiActive = true, guiActiveEditor = true, active = true, guiName = "Select Team")]
        ...
        public void UpdateList()
        {
            ...
            while (weapon.MoveNext())
            {
				...
				if (weapon.Current.GetWeaponClass() == WeaponClasses.Rocket &&
					...
					&& !weapon.Current.GetPart().FindModuleImplementing<ModuleWeapon>().externalAmmo && !BDArmorySettings.INFINITE_AMMO)
				{
					continue;
                }
				...
				{
					weapon.Current.GetPart().FindModuleImplementing<MissileBase>().GetMissileCount(); // #191, Do it this way so the GetMissileCount enum isn't called every frame
				}
            }
            weapon.Dispose();
            ...
            else if (target.isFlying)
            {
                ...
                while (item.MoveNext())
                {
					...
					if (candidateClass == WeaponClasses.Rocket)
					{
						...
						if ((targetWeapon != null) && (targetRocketPower > canidateRocketThrust))
							continue; //don't replace faster rockets
						targetWeapon = item.Current;
						targetRocketPower = canidateRocketThrust;
					}
            ...
            else if (target.isLandedOrSurfaceSplashed)
            {
                ...
                while (item.MoveNext())
                {
					...
					if (candidateClass == WeaponClasses.SLW && target.isSplashed)
					{
						...
						{
							if (targetWeapon != null || droptime <= 0) continue; // don't fire sts torps from planes
						...
						if (vessel.Splashed)
						{
							...
							targetYield = canidateYield;
							targetWeapon = item.Current;
							if (targetWeapon != null && distance > canidateYield) // don't fire torps when within blast radius
								break;  //prioritize torps against naval targets
						}
					}
        ...
        void GuardMode()
        {
			...
			if (!guardTarget && selectedWeapon != null && (selectedWeapon.GetWeaponClass() == WeaponClasses.Gun || selectedWeapon.GetWeaponClass() == WeaponClasses.Rocket))
			{
				List<ModuleWeapon>.Enumerator weapon = vessel.FindPartModulesImplementing<ModuleWeapon>().GetEnumerator();
                while (weapon.MoveNext())
                {
