...
namespace BDArmory.Modules
{
    public class MissileLauncher : MissileBase
    {
        ...
        void Update()
        {
			...
			if (HighLogic.LoadedSceneIsFlight)
			{
				if (weaponClass == WeaponClasses.SLW && FlightGlobals.getAltitudeAtPos(part.transform.position) > 0) //#710
				{
					...
					dropTime = ((float)Math.Sqrt(a * (a + (8 * d))) - a) / (2 * a) - (Time.fixedDeltaTime * 1.5f);
				}
			}
		}
        ...
        void UpdateThrustForces()
        {
			...
			if (weaponClass == WeaponClasses.SLW && FlightGlobals.getAltitudeAtPos(part.transform.position) > 0) return; //#710
        ...
        IEnumerator BoostRoutine()
        {
            ...
            while (Time.time - boostStartTime < boostTime)
            {
                ...
                while (gpe.MoveNext())
                {
                    ...
                    if ((!vessel.InVacuum() && Throttle > 0) && weaponClass != WeaponClasses.SLW || (weaponClass == WeaponClasses.SLW && FlightGlobals.getAltitudeAtPos(part.transform.position) < 0 ))
                    {
